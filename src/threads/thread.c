...
bool
compare_actual_priority(const struct list_elem * a,const struct list_elem * b,void *aux){
    return list_entry(a,struct thread,elem)->actual_priority > list_entry(b,struct thread,elem)->actual_priority;
}
...
thread_set_priority (int new_priority)
{
    ...
    if(!thread_mlfqs){//fifth error place
    struct thread *cur = thread_current ();
    ASSERT(cur!=NULL);
    if(cur->actual_priority > new_priority)
    {
        if(cur->priority == cur->actual_priority)
        {
